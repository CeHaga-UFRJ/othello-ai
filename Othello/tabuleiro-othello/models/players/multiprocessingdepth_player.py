...
class MultiprocessingdepthPlayer:
    ...
    class MinMaxProcess(Process):
        def __init__(self, board, depth, color, alpha, beta, player, queue):
        ...
        def run(self):
            score, move = self.player.minmax(
                self.board, self.depth, self.color, self.alpha, self.beta)
            self.queue.put((score, move))
    ...
    def play(self, board):
        ...
        queue = Queue()
        pid = 0
        max_depth = self.starting_depth + self.cpu
        processes = [self.MinMaxProcess(board.get_clone(), depth, board._opponent(
            self.color), -math.inf, math.inf, self, queue) for depth in range(self.starting_depth, max_depth)]
        ...
        while not time_over:
            ...
            best_move = move
            best_score = score
            processes.append(self.MinMaxProcess(board.get_clone(), max_depth, board._opponent(
                self.color), -math.inf, math.inf, self, queue))
            processes[-1].start()
            max_depth += 1
            pid += 1
