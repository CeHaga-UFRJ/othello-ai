...
class MultiprocessingPlayer:
    ...
    class MinMaxProcess(Process):
        def __init__(self, board, depth, color, alpha, beta, player, move, queue):
        ...
        def run(self):
            self.score, _ = self.player.minmax(
                self.board, self.depth, self.color, self.alpha, self.beta)
            self.queue.put((self.depth, self.score, self.move))
    ...
    def play(self, board):
        ...
        for depth in range(self.starting_depth, self.depth):
            ...
            for move in valid_moves:
                ...
                threads.append(self.MinMaxProcess(copy_board, depth, board._opponent(
                    self.color), -math.inf, math.inf, self, move, queue))
